%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function C2C12_mod_INT_FA_F3_V1(ctob, varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Module to determine intensity of FAs
% Argin
%   ## control
%   ctob:   obj switch
%   ctcal:  job switch - to calculate
%   ctplot: job switch - to plot bnd
%   ctplcp: job switch - to plot cropped bnd
%   ctxls:  job switch - to write xls
%   ## switch
%   swbd:  switch - to plot bnd {1}
%   swtx:  switch - to plot bnd # {1}
%   swscb: switch - to plot scale bar {1}
%   swval: switch - to plot value {1}
%   ## img
%   imgco:  image - fluorescence color {'g'}
%   sat: saturation level {[0.01 0.99]}
%   gam: gamma {1.0}
%   ## bnd
%   lnty:   bnd   - line type {'--'}
%   lnco:   bnd   - line color {[0.99 0.99 0.99]}
%   lnwd:   bnd   - line width {0.5}
%   ## bnd #
%   txco:   bnd # - text color {[0.99 0.99 0.99]}
%   txsz:   bnd # - text size {20}
%   ## plot
%   cmaxv:  color limit - {0}
% Author
%   Sung Sik Hur - sstahur@gmail.com
%   Last Modified - 220421

% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %%% [1] Pre-Process
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% (1) Default
co_gr=0.5*[1 1 1];
pnm = {...
    'ctcal', 'ctplot', 'ctplcp','ctxls',  ... %%% ctl
    'swbd', 'swtx', 'swscb','swval',      ... %%% switch
    'sat', 'gam', 'imgco',                ... %%% img
    'lnty', 'lnwd', 'lnco', 'lnap',       ... %%% bnd
    'txsz', 'txco', 'txap',               ... %%% bnd #
    'bwd', 'bco', 'bap',                  ... %%% scalebar
    'cmaxv',                              ... %%% plot
    'flectl', 'flefle', 'fledat'          ... %%% var
    };
pdf = {...
    0,0,0,0,                              ... %%% ctl
    1, 1, 1, 1,                           ... %%% switch
    [0.01 0.99], 1.0, 'g',                ... %%% img
    '--', 0.5, co_gr, 0.5,                ... %%% bnd
    20, co_gr,1.0,                        ... %%% bnd #
    5, co_gr, 0.25,                       ... %%% scalebar
    0,                                    ... %%% plot  
    [], [], []                            ... %%% var
    };
[ctcal, ctplot, ctplcp,ctxls,             ... %%% ctl
    swbd, swtx, swscb,swval,              ... %%% switch
    sat, gam, imgco,                      ... %%% img
    lnty, lnwd, lnco, lnap,               ... %%% bnd
    txsz, txco, txap,                     ... %%% bnd #
    bwd, bco, bap,                        ... %%% scalebar
    cmaxv,                                ... %%% plot
    flectl, flefle, fledat                ... %%% var
    ] = parse_param(pnm, pdf, varargin);

% (2) Control
ctall=logical(sum([ctcal, ctplot, ctplcp, ctxls]));if ctall==0; return; end %%% check control

% (2) Load
% 1 control
exc=loadfle(flectl,'mat', 'exc');

% 2 file
[fleIMGc,                                                                       ... %%% img
fleBNDmatc, fleBNDfigc, fleBNDfg1c, fleBNDfg2c, fleBNDfg3c,                     ... %%% bnd
fleBD2matc, fleBD2figc, fleBD2fg1c, fleBD2fg2c, fleBD2fg3c,                     ... %%% bnd2
fleMORmatc, fleMORfigc, fleMORfg1c, fleMORfg2c, fleMORfg3c, fleMORxls,          ... %%% mor
fleMO2matc, fleMO2figc, fleMO2fg1c, fleMO2fg2c, fleMO2fg3c, fleMO2xls,           ... %%% mor2
fleINTmatc, fleINTfigc, fleINTfg1c, fleINTfg2c, fleINTfg3c, fleINTxls,          ... %%% int
fleIT2matc, fleIT2figc, fleIT2fg1c, fleIT2fg2c, fleIT2fg3c, fleIT2xls           ... %%% it1
]...
=loadfle(flefle,'mat', ...
'fleIMGc',                                                                      ... %%% img
'fleBNDmatc', 'fleBNDfigc', 'fleBNDfg1c', 'fleBNDfg2c', 'fleBNDfg3c',           ... %%% bnd
'fleBD2matc', 'fleBD2figc', 'fleBD2fg1c', 'fleBD2fg2c', 'fleBD2fg3c',           ... %%% bnd1
'fleMORmatc', 'fleMORfigc', 'fleMORfg1c', 'fleMORfg2c', 'fleMORfg3c','fleMORxls',... %%% mor
'fleMO2matc', 'fleMO2figc', 'fleMO2fg1c', 'fleMO2fg2c', 'fleMO2fg3c', 'fleMO2xls',... %%% mor1
'fleINTmatc', 'fleINTfigc', 'fleINTfg1c', 'fleINTfg2c', 'fleINTfg3c', 'fleINTxls',  ... %%% int
'fleIT2matc', 'fleIT2figc', 'fleIT2fg1c', 'fleIT2fg2c', 'fleIT2fg3c', 'fleIT2xls'   ... %%% it1
);

% 3 Variables
[imsz, imszum, imszcp, imszcpum, pxsz ...           %%% img
bsz, bszum, bszcp, bszcpum ...                      %%% scalebar
]=loadfle(fledat,'mat', ...
'imsz', 'imszum', 'imszcp', 'imszcpum', 'pxsz',...  %%% img
'bsz', 'bszum', 'bszcp', 'bszcpum' ...              %%% scalebar
);
H=imsz(1);W=imsz(2);
Hcp=imszcp(1);Wcp=imszcp(2);

% (3) Parameter
% 1 cal
bgra=0.01;

% 2 plot
%%% switch
swmaxfig = 0;
swcb=1;
optight='loose';

%%% img
load('const_sh');
imgco_r = COR.tomato;
imgco_g = COG.lightgreen;
imgco_b = COB.skyblue;
satd=[0.001 0.999];

%%% int
cmap=parula;
% cmap=jetn;
txal='center';

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% [2] Process
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
ii=0;
jj=0;
nr=size(ctob,1);
for i=1:nr
    if ctob(i)
        % (1) Parameter
        % 1 para
        ex=exc{i,1};
        fprintf('%s\n', ex);
        
        % 2 file
        %%% img
        fleim_r  = fleIMGc{i,1};
        fleim_g  = fleIMGc{i,2};
        fleim_b  = fleIMGc{i,3};
        %%% bnd
        flebndmat = fleBNDmatc{i}; % Cell
        flebd2mat = fleBD2matc{i}; % FA
        %%% mor
        flemormat = fleMORmatc{i}; % Cell
        flemo2mat = fleMO2matc{i}; % FA
        %%% int
        fleintmat = fleINTmatc{i};
        fleit2mat = fleIT2matc{i};
        fleit2fig = fleIT2figc{i};
        fleit2fg1 = fleIT2fg1c{i};
        %%% para
        cmax=cmaxv(i); % color
        
        % (2) Load
        %%% img
        im_r=imreadfle(fleim_r,'tif');imaj_r=imad(im_r,sat, gam);imco_r=imgray2rgb(imaj_r,imgco_r);
        im_g=imreadfle(fleim_g,'tif');imaj_g=imad(im_g,sat, gam);imco_g=imgray2rgb(imaj_g,imgco_g);
        im_b=imreadfle(fleim_b,'tif');imaj_b=imad(im_b,sat, gam);imco_b=imgray2rgb(imaj_b,imgco_b);
        imco_rgb=cat(3, imaj_r, imaj_g, imaj_b);
        
        %%% bnd - cell
        [tbdc, rbdc, thsv]=loadfle(flebndmat,'mat', 'tbdc', 'rbdc', 'thsv'); % Cell
        %%% bnd - FA
        [bdokcc, bwokc, stokc,              ... %%% FA ok
                rbdc, thsv,                 ... %%% rbd & ths
                bdlgcc, bwlgc, stlgc,       ... %%% FA large
                bdsmcc, bwsmc, stsmc        ... %%% FA small
            ]=loadfle(flebd2mat,'mat',      ...
                'bdokcc', 'bwokc', 'stokc', ... %%% FA ok
                'rbdc', 'thsv',               ... %%% rbd & ths
                'bdlgcc', 'bwlgc', 'stlgc',  ... %%% FA large
                'bdsmcc', 'bwsmc', 'stsmc'   ... %%% FA small
                );
        bdc_cell = tbdc;
        [~, bwc_cell]=bd2bwmaxc(tbdc, imsz);
        bdcc_FA = bdokcc;
        bwc_FA  = bwokc;
        nbd=length(tbdc);
        
        % (3) Cal
        if ctcal==0
            [ita, itmc, itsc, itmv, itsv]=loadfle(fleit2mat,'mat', 'ita', 'itmc', 'itsc', 'itmv', 'itsv');
        end
        if ctcal==1
            im=im_g;
            
            [ita, itmc, itsc, itmv, itsv] = calitfac(im, bwokc, bgra);
            savefle(fleit2mat,'mat', ita, itmc, itsc, itmv, itsv)
        end
        
        % (4) Plot
        if cmax==0;clm=0;else;clm=[0 cmax];end
        if ctplot==1
            %%% im
            im=ita;
            %%% bd
            bd1c  = bdc_cell;
            bw1c  = bwc_cell;
            bd2cc = bdcc_FA;
            bw2c  = bwc_FA;
            
            %%% plot
            imshowmap(im, 'cmap', cmap, 'swcb', swcb, 'clm', clm);hold on;axis tight;
            if swbd==1;plotbdc(bd1c, 'lnty',lnty, 'lnwd', lnwd, 'lnco', [lnco, lnap], 'swtx', 0, 'txsz', txsz, 'txco', [txco, txap]);end
            
            %plotbdc(nbdc, 'lnty',lnty, 'lnco', [lnco, lnap], 'lnwd', lnwd, 'swtxt', swtxt, 'txsz', txsz, 'txco', txco);
            %if swval==1; for k=1:nbdtextxy(mean(bd1c{k},1), sprintf('it\n%0.2g',itsv(k)), 'txco', txco, 'txsz', txsz, 'txal', txal);end;end
            if swscb==1;addscb([W*(1-0.03)-bsz, H*(1-0.05)], bsz, 'bco', [bco, bap], 'bwd', bwd);end; axis off;
            axcon1('xlm', [0 W], 'ylm', [0 H], 'xt', [], 'yt', [], 'optight', optight);box on;
            exportgraphics(gca,sprintf('%s.jpg',fleit2fig),'Resolution',300);hold off;close;
        end
        
        % (5) Plot Crop
        if ctplcp==1
            %%% img
            im=ita;
            %%% bd
            bd1c  = bdc_cell;
            bw1c  = bwc_cell;
            bd2cc = bdcc_FA;
            bw2c  = bwc_FA;
            
            for k=1:nbd
                %%% bd
                bd1  = bd1c{k};
                bw1  = bw1c{k}; 
                bd2c = bd2cc{k};
                bw2  = bw2c{k};
                %%% rec
                [xlm,ylm, xlmum, ylmum]=recbd(bd1, imszcp, imsz, pxsz);
                %%% im bd
                imbd = getimbw(im, bw2);
                
                %%% plot
                imshowmap(imbd, 'cmap', cmap, 'swcb', swcb, 'clm', clm);hold on;axis tight;
                if swbd==1;plotbd(bd1, 'lnty',lnty, 'lnwd', lnwd, 'lnco', [lnco, lnap], 'swtx', 0, 'txsz', txsz, 'txco', [txco, txap]);end
                if swval==1;textxy(mean(bd1), sprintf('it#%d\n%0.2g',k, itsv(k)), 'txco', txco, 'txsz', txsz, 'txal', txal);end
                if swscb==1;addscb([xlm(2)-bszcp-Wcp*0.05, ylm(2)-Hcp*0.05], bszcp, 'bco', [bco, bap], 'bwd', bwd);end
                axcon1('xlm', xlm, 'ylm', ylm, 'xt', [], 'yt', [], 'optight', optight);
                exportgraphics(gca,sprintf('%s-%d.jpg',fleit2fg1, k),'Resolution',300);hold off;close;
            end
        end

        % (6) xls list
        if ctxls==1
            for k=1:nbd
                itm=itmc{k};
                its=itsc{k};
                nob=length(itm);
                for m=1:nob
                    % object out
                    ii=ii+1;
                    lex{ii,1} = sprintf('e%ds%dc%do%d', exc{i,3}, exc{i,4}, k, m);
                    lep(ii,1) = exc{i,3};
                    lsp(ii,1) = exc{i,4};
                    lcp(ii,1) = k;
                    lop(ii,1) = m;
                    litm(ii,1) = itm(m);
                    lits(ii,1) = its(m);
                end
                % cell out
                jj=jj+1;
                vex{jj,1} = sprintf('e%ds%dc%d', exc{i,3}, exc{i,4}, k);
                vep(jj,1) = exc{i,3};
                vsp(jj,1) = exc{i,4};
                vcp(jj,1) = k;
                vitm(jj,1) = itmv(k); 
                vits(jj,1) = itsv(k); 
            end
        end
    end
end
if ctxls==1
    % (5) xls list
    % 1 parameter
    flexls=fleIT2xls;
    clear H1 H2 H3 B1 B2 B3 res;
    H1={'ex'};
    H2={'ep', 'sp','cp', 'op'};
    H3={'itm', 'its'};
    B1=lex;
    B2=[lep, lsp, lcp, lop];B2=num2cell(B2);
    B3=[litm, lits];B3=num2cell(B3);
    res=[H1, H2, H3;B1, B2, B3];
    sht='ALL obj';saveflexls(flexls, 'xlsx', sht, res);

    % (2) LIST FA mean
    clear H1 H2 H3 B1 B2 B3 res;
    H1={'ex'};
    H2={'ep', 'sp','cp'};
    H3={'itm', 'its'};
    B1=vex;
    B2=[vep, vsp, vcp];B2=num2cell(B2);
    B3=[vitm, vits];B3=num2cell(B3);
    res=[H1, H2, H3;B1, B2, B3];
    sht='ALL mean';saveflexls(flexls, 'xlsx', sht, res);
    
    % (3) TABLE FA obj
    clear H1 H2 H3 B1 B2 B3 res;
    % 1 data
    gv=[lep, lsp, lcp];
    g1=1:max(lep(:));n1=length(g1);
    g2=1:max(lsp(:));n2=length(g2);
    g3=1:max(lcp(:));n3=length(g3);
    gic={g1, g2, g3};
    [mep, msp, mcp, mop, mitm,mits, nm]=v2m_3Dmt(gv, gic, lep, lsp, lcp, lop, litm, lits);
    nmax=max(nm(:));
    [mep, msp, mcp, mop, mitm,mits] = permutemt([3 2 1 4],        mep, msp, mcp, mop, mitm,mits); %to change dimensions
    [mep, msp, mcp, mop, mitm,mits] = reshapemt([n1*n2*n3, nmax], mep, msp, mcp, mop, mitm,mits);
    [mep, msp, mcp, mop, mitm,mits] = transposemt(                mep, msp, mcp, mop, mitm,mits);
    % find nan col and remove
    iempty=logical(prod(isnan(mitm),1)); % empty col index
    [mep, msp, mcp, mop, mitm,mits]=rmcolmt(iempty,               mep, msp, mcp, mop, mitm,mits);
    
    % 2 head
    nr=size(mep,1);
    nc=size(mep,2);
    % head
    [H1, H3]=deal(cell(1,nc));
    H2={'n'};
    for j=1:nc
        if ~isnan(mep(1,j))
            H1{1,j}=sprintf('e%ds%dc%d', mep(1,j), msp(1,j), mcp(1,j));
            H3{1,j}=sprintf('e%ds%dc%d', mep(1,j), msp(1,j), mcp(1,j));
        end
    end
    % 3. Body
    B1=cell(nr,nc);
    B2=cell(nr,1);
    for i=1:nr
        for j=1:nc
            if ~isnan(mep(i,j))
                B1{i,j}=sprintf('e%ds%dc%do%d',mep(i,j),msp(i,j), mcp(i,j), mop(i,j));
            end
        end
    end
    for i=1:nr
        B2{i,1}=sprintf('n%d', i);
    end
    % 4. write
    B3=num2cell(mep);sht='ep_o';  res=[H1,H2 H3;B1,B2,B3];saveflexls(flexls, 'xlsx', sht, res);
    B3=num2cell(msp);sht='sp_o';  res=[H1,H2 H3;B1,B2,B3];saveflexls(flexls, 'xlsx', sht, res);
    B3=num2cell(mcp);sht='cp_o';  res=[H1,H2 H3;B1,B2,B3];saveflexls(flexls, 'xlsx', sht, res);
    B3=num2cell(mop);sht='op_o';  res=[H1,H2 H3;B1,B2,B3];saveflexls(flexls, 'xlsx', sht, res);
    B3=num2cell(mitm);sht='itm_o';  res=[H1,H2 H3;B1,B2,B3];saveflexls(flexls, 'xlsx', sht, res);
    B3=num2cell(mits);sht='its_o';  res=[H1,H2 H3;B1,B2,B3];saveflexls(flexls, 'xlsx', sht, res);
    
    % (4) TABLE FA mean
    clear H1 H2 H3 B1 B2 B3 res;
    % 1 data
    gv=vep;
    g1=1:max(vep(:));n1=length(g1);
    gi=g1;
    [pep, psp, pcp, pitm,pits]=v2m_1Dmt(gv, gi, vep, vsp, vcp, vitm, vits);
    [pep, psp, pcp, pitm,pits]=transposemt(             pep, psp, pcp, pitm,pits);
    % find nan col and remove
    iempty=logical(prod(isnan(pitm),1)); % empty col index
    [pep, psp, pcp, pitm,pits]=rmcolmt(iempty,          pep, psp, pcp, pitm,pits);

    % 2 head
    nr=size(pep,1);
    nc=size(pep,2);
    [H1, H3]=deal(cell(1,nc));
    H2={'n'};
    for j=1:nc
        if ~isnan(pep(1,j))
            H1{1,j}=sprintf('e%ds%d', pep(1,j));
            H3{1,j}=sprintf('e%ds%d', pep(1,j));
        end
    end
        
    % 3. Body
    B1=cell(nr,nc);
    B2=cell(nr,1);
    for i=1:nr
        for j=1:nc
            if ~isnan(pep(i,j))
                B1{i,j}=sprintf('e%ds%dc%d',pep(i,j),psp(i,j), pcp(i,j));
            end
        end
    end
    for i=1:nr
        B2{i,1}=sprintf('n%d', i);
    end
    % 4. write
    B3=num2cell(pep);sht='ep_m';  res=[H1,H2 H3;B1,B2,B3];saveflexls(flexls, 'xlsx', sht, res);
    B3=num2cell(psp);sht='sp_m';  res=[H1,H2 H3;B1,B2,B3];saveflexls(flexls, 'xlsx', sht, res);
    B3=num2cell(pcp);sht='cp_m';  res=[H1,H2 H3;B1,B2,B3];saveflexls(flexls, 'xlsx', sht, res);
    B3=num2cell(pitm);sht='itm_m';  res=[H1,H2 H3;B1,B2,B3];saveflexls(flexls, 'xlsx', sht, res);
    B3=num2cell(pits);sht='its_m';  res=[H1,H2 H3;B1,B2,B3];saveflexls(flexls, 'xlsx', sht, res);
end
